    ...
    <section className="relative w-full">
      ...
      <div
        className={`absolute inset-0 w-full h-full transition-opacity duration-[400ms] ${
          loaded ? "opacity-100" : "opacity-0 translate-y-[10px]"
        }`}
      >
        <div className="mx-auto max-w-[1300px] px-10 h-full flex items-center justify-center">
          ...
          <div className="text-center flex flex-col items-center gap-4 md:gap-5">
            <p
              className={`font-sans text-sm md:text-base uppercase tracking-[0.3em] text-white/80 ${
                loaded ? "appear-visible" : "appear-hidden"
              }`}
            >
              Welcome To The Mindset
            </p>
            <h1
              className={`font-heading text-[40px] md:text-[72px] leading-[1.05] text-white tracking-[2px] font-normal ${
                loaded ? "appear-visible" : "appear-hidden"
              }`}
            >
              RICH MINDSET
            </h1>
            <div className={loaded ? "appear-visible" : "appear-hidden"}>
              <Link
                href="#products"
